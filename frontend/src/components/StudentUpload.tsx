...
const StudentUpload = () => {
  ...
  const handlePointsChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    setForm((prev) => ({ ...prev, points: parseInt(e.target.value) }));
  };
  ...
  return (
    <div className="p-5">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <label>Topic</label>
        <input name="topic" value={form.topic} onChange={handleChange} className="w-full p-2 border rounded" />

        <label>Subject</label>
        <input name="subject" value={form.subject} onChange={handleChange} className="w-full p-2 border rounded" />

        <label>Lesson Number</label>
        <input name="lessonNumber" value={form.lessonNumber} onChange={handleChange} className="w-full p-2 border rounded" />

        <label>Points</label>
        <select name="points" value={form.points} onChange={handlePointsChange} className="w-full p-2 border rounded">
          {[50, 100, 200, 500, 1000].map((point) => (
...
Correct Answer`}
      ...
      <div className="mt-5">
        {pathways.map((p) => (
          <div key={p._id} className="p-4 border rounded">
            <h2 className="text-lg font-bold">{p.topic}</h2>
            <p>{p.subject}</p>
            <button onClick={() => handleEdit(p)} className="bg-yellow-500 text-white px-3 py-1 rounded">Edit</button>
            <button onClick={() => handleDelete(p._id!)} className="bg-red-500 text-white px-3 py-1 rounded ml-2">Delete</button>
          </div>
